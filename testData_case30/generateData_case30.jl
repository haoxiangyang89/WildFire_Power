...
network_data = PowerModels.parse_file("data/case30/case30.m")
...
function prepareIndexSets(network_data::Dict{String, Any} ,T::Int64, Ω::Int64; 
    ...
    for i in keys(network_data["load"])
        ...
        push!(D, d)
        for t in 1:T 
            demand = network_data["load"][i]["pd"]                    ## https://github.com/GridMod/RTS-GMLC/tree/master/RTS_Data/timeseries_data_files
            Demand[t][d] = demand
        end
    end
    ...
    for i in keys(network_data["gen"])
        ...
        cg[g] = wsample([50, 1000, 2500], [0.2, .75, 0.05], 1)[1]                                   ############# need to revis
    end
...
save("src/testData_case30/sampleSize4/indexSets.jld2", "indexSets", indexSets)
save("src/testData_case30/sampleSize4/paramOPF.jld2", "paramOPF", paramOPF)
save("src/testData_case30/sampleSize4/paramDemand.jld2", "paramDemand", paramDemand)
save("src/testData_case30/sampleSize4/Ω_rv.jld2", "Ω_rv", Ω_rv)
save("src/testData_case30/sampleSize4/prob.jld2", "prob", prob)


## Case 30
indexSets = load("src/testData_case30/sampleSize4/indexSets.jld2")["indexSets"]
paramOPF = load("src/testData_case30/sampleSize4/paramOPF.jld2")["paramOPF"]
paramDemand = load("src/testData_case30/sampleSize4/paramDemand.jld2")["paramDemand"]
Ω_rv = load("src/testData_case30/sampleSize4/Ω_rv.jld2")["Ω_rv"]
prob = load("src/testData_case30/sampleSize4/prob.jld2")["prob"]